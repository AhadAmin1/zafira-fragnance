    ...
    <>
      ...
      <nav 
        className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 ${
          isLightMode 
            ? 'bg-cream/95 backdrop-blur-md shadow-sm border-b border-gold/10 h-20 md:h-24' 
          ...
          <div className="flex items-center justify-between h-full">
            ...
            <Link to="/" onClick={closeMenu} className="flex items-center gap-3 group relative z-[1001]">
              <div className="relative flex items-center justify-center w-10 h-10 md:w-14 md:h-14">
                ...
                <svg viewBox="0 0 24 24" className="w-6 h-6 md:w-9 md:h-9 fill-gold drop-shadow-md group-hover:scale-110 transition-transform duration-500" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="flex flex-col -gap-1">
                <span className={`font-display font-bold tracking-[0.25em] leading-none transition-all duration-500 ${isLightMode ? 'text-xl md:text-3xl text-charcoal' : 'text-2xl md:text-4xl text-white'}`}>ZAFIRA</span>
                <span className={`text-[8px] md:text-xs font-body tracking-[0.5em] uppercase leading-none mt-1 transition-colors duration-500 ${isLightMode ? 'text-gold' : 'text-gold-light'}`}>Fragrances</span>
              </div>
            </Link>

            {/* Desktop Nav Links */}
            <div className="hidden md:flex items-center gap-10">
              <NavLink to="/" className={({ isActive }) => `relative font-body text-xs tracking-[0.2em] uppercase transition-all duration-300 group ${isActive ? 'text-gold' : isLightMode ? 'text-charcoal hover:text-gold' : 'text-white/80 hover:text-white'}`}>
                Home
                <span className="absolute -bottom-1 left-0 w-0 h-px bg-gold transition-all duration-300 group-hover:w-full"></span>
              </NavLink>
              <NavLink to="/products" className={({ isActive }) => `relative font-body text-xs tracking-[0.2em] uppercase transition-all duration-300 group ${isActive ? 'text-gold' : isLightMode ? 'text-charcoal hover:text-gold' : 'text-white/80 hover:text-white'}`}>
                Collection
                <span className="absolute -bottom-1 left-0 w-0 h-px bg-gold transition-all duration-300 group-hover:w-full"></span>
              </NavLink>
              <Link to="/cart" className="relative group">
                <div className={`flex items-center gap-2 font-body text-xs tracking-[0.2em] uppercase transition-all duration-300 ${isLightMode ? 'text-charcoal group-hover:text-gold' : 'text-white/80 group-hover:text-white'}`}>
                  <div className="relative">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                    </svg>
                    {totalItems > 0 && (
                      <span className="absolute -top-2 -right-2 bg-gold text-white text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold animate-bounce">
                        {totalItems}
                      </span>
                    )}
                  </div>
